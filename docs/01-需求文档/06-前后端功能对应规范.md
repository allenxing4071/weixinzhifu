...
**基于已完成并正常运行的功能，确保小程序前端与后端API功能完全对应，指导后续开发严格按现有架构扩展。**
...
|---------|---------|-----------|---------|----------|----------|
| 用户登录 | `POST /api/v1/auth/wechat-login` | app.js | ✅ 生产模式 | ✅ 已实现 | 🧪 待测试 |
| 积分余额 | `GET /api/v1/points/balance` | pages/points/index | ✅ 真实API调用 | ✅ 已实现 | 🧪 待测试 |
| 积分历史 | `GET /api/v1/points/history` | pages/points/index | ✅ 真实API调用 | ✅ 已实现 | 🧪 待测试 |
| 商户信息 | `GET /api/v1/merchants/:id` | pages/payment/index | ✅ loadRealMerchantInfo | ✅ 已实现 | 🧪 待测试 |
| 创建支付 | `POST /api/v1/payments/create` | pages/payment/index | ✅ 真实微信支付 | ✅ 已实现 | 🧪 待测试 |
| 用户信息 | `GET /api/v1/auth/user-info` | pages/profile/index | ✅ 真实API调用 | ✅ 已实现 | 🧪 待测试 |
